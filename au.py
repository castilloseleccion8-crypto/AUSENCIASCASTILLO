...
st.set_page_config(page_title="Gestion de Ausencias", layout="wide")
...
NAALOO_OPTS = [
    "Si, esta cargado en Naaloo",
    "No, falta cargarlo en Naaloo",
    "No aplica",
]
PASSWORD_RRHH  = "Tucuman2026"
...
def form_tipificacion(key, gestion_actual, gestiones_dict, sucursal_origen):
    ...
    obs  = st.text_area("Observaciones", value=gestion_actual.get("observaciones",""),
                        placeholder="Ej: Aviso por WhatsApp, presento certificado el...",
    ...
    if gestion_actual.get("fecha_gestion"):
        st.caption(f"Ultimo guardado: {gestion_actual['fecha_gestion']} — {gestion_actual.get('responsable','')} ({gestion_actual.get('sucursal_origen','')})")
...
with st.sidebar:
    ...
    if modo == "Sucursal":
        ...
        sucursal_sel = None
        password     = st.text_input("Contrasena RRHH", type="password")
        es_rrhh      = True
...
# HEADER
st.markdown('<h1 style="text-align:center;color:#0f1f3d;font-size:32px;font-weight:800;margin-bottom:4px;">GESTION DE AUSENCIAS</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align:center;color:#666;font-size:15px;margin-bottom:20px;">Tipificacion y seguimiento diario</p>', unsafe_allow_html=True)

if not password:
    st.info("Selecciona tu sucursal e ingresa la contrasena para continuar.")
    st.stop()
if not autenticado:
    st.stop()
...
# PANEL RRHH
if es_rrhh:
    with st.expander("Administracion", expanded=df_global.empty):
        tab_subir, tab_limpiar = st.tabs(["Subir nuevo reporte", "Archivar casos"])

        with tab_subir:
            if not df_global.empty:
                st.markdown("**Borrar reporte actual**")
                st.caption("Elimina todos los datos del reporte. Las tipificaciones guardadas NO se borran.")
                col_del, _ = st.columns([1, 3])
                with col_del:
                    if st.button("Borrar todo el reporte", key="btn_borrar_reporte"):
                        with st.spinner("Borrando..."):
                            ws_rep = get_sheet(HOJA_REPORTE)
                            ws_rep.clear()
                        st.success("Reporte borrado. Ya podes subir el nuevo Excel.")
                        st.cache_data.clear()
                        st.rerun()
                st.markdown("---")

            st.markdown("**Subir nuevo Excel**")
            ...
            if archivo is not None:
                try:
                    ...
                    st.success(f"{len(df_nuevo)} filas leidas — {len(aus_n)} ausentes detectados")
                    st.dataframe(df_nuevo.head(8), use_container_width=True, hide_index=True)
                    c_btn, _ = st.columns([1, 3])
                    with c_btn:
                        if st.button("Confirmar y subir", key="btn_subir"):
        ...
        with tab_limpiar:
            if df_global.empty:
                ...
                st.markdown("---")
                st.markdown("**O selecciona casos individuales:**")
                ...
                sel_todo = st.checkbox(f"Seleccionar todos ({len(df_adm_v)})", key="chk_todo")
                ...
                for i, (orig_idx, row) in enumerate(df_adm_v.iterrows()):
                    ...
                    g   = gestiones.get(k, {})
                    if est == "RESUELTO":
                        est_txt = f"Tipificado: {g.get('tipificacion','')}"
                    elif est == "VENCIDO":
                        est_txt = "VENCIDO"
                    else:
                        est_txt = "PENDIENTE"

                    pos_txt = f" · {str(row.get('POSICION','')).strip()}" if str(row.get('POSICION','')).strip() not in ("","nan") else ""

                    cc, ci = st.columns([0.5, 9.5])
                    with cc:
                        elegido = st.checkbox("", key=f"chk_{k}_{i}", value=sel_todo, label_visibility="collapsed")
                    with ci:
                        st.markdown(f"**{row['NOMBRE'].strip()}**{pos_txt} · Leg. {row['LEGAJO']} · {row['SUCURSAL']} · {row['FECHA']} · {est_txt}")
                    if elegido:
                        seleccionados.append((orig_idx, k))

                if seleccionados:
                    st.warning(f"{len(seleccionados)} caso(s) seleccionados. Se moveran al HISTORIAL.")
                    cd, _ = st.columns([1, 4])
                    with cd:
                        if st.button("Archivar seleccionados", key="btn_arch"):
                            with st.spinner("Archivando..."):
                                archivar_y_eliminar(
                            ...
                            st.success("Archivados correctamente.")
                            st.cache_data.clear()
                            st.rerun()
...
if df_filtrado.empty:
    ...
    for (legajo, nombre, sucursal), grupo in grupos:
        ...
        dias_txt = f"{dias_grupo} dia{'s' if dias_grupo > 1 else ''} en falta"
        suc_txt  = f"  ·  {sucursal}" if es_rrhh else ""
        pos_txt  = f"  ·  {posicion}" if posicion and posicion not in ("nan","") else ""
        ...
        for _, row in grupo.iterrows():
            ...
            if est_actual == "RESUELTO":
                ...
                dia_badge = f'<span style="background:#d1fae5;color:#065f46;padding:1px 8px;border-radius:12px;font-size:11px;">Tipificado: {tip_txt}</span>'
                ...
                dr_txt    = f"{dias_restantes} dia{'s' if dias_restantes != 1 else ''}" if dias_restantes > 0 else "vence hoy"
                dia_badge = f'<span style="background:#fef3c7;color:#92400e;padding:1px 8px;border-radius:12px;font-size:11px;">Pendiente — {dr_txt}</span>'
            ...
            st.markdown(
                f'&nbsp;&nbsp;&nbsp;Fecha: **{row["FECHA"]}** &nbsp; '
                f'Manana: **{fmt_fichada(row["MAÑANA"])}** · Tarde: **{fmt_fichada(row["TARDE"])}** '
                f'&nbsp; {dia_badge}{lic_html}',
                unsafe_allow_html=True
            )

            label_exp = f"Ver tipificacion — {row['FECHA']}" if est_actual == "RESUELTO" else f"Tipificar — {row['FECHA']}"
